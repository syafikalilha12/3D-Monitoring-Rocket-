...
public class D3DEnumeration
{
	...
	/// </summary>
	static int GetColorChannelBits(Format format)
	{
		switch (format)
		{
			case Format.R8G8B8:
	...
	/// </summary>
	static int GetAlphaChannelBits(Format format)
	{
		switch (format)
		{
			case Format.R8G8B8:
	...
	/// </summary>
	static int GetDepthBits(DepthFormat format)
	{
		switch (format)
		{
	...
	/// </summary>
	static int GetStencilBits(DepthFormat format)
	{
		switch (format)
		{
			case DepthFormat.D16:
		...
		foreach (AdapterInformation ai in Manager.Adapters)
		{
		    ...
		    foreach (DisplayMode displayMode in ai.SupportedDisplayModes)
		    {
			    ...
			    if (displayMode.Height < AppMinFullscreenHeight)
				    continue;
			    if (GetColorChannelBits(displayMode.Format) < AppMinColorChannelBits)
				    continue;
	...
	protected void EnumerateDeviceCombos(GraphicsDeviceInfo deviceInfo, ArrayList adapterFormatList)
	{
		...
		foreach (Format adapterFormat in adapterFormatList)
		{
			...
			{
				if (GetAlphaChannelBits(backBufferFormat) < AppMinAlphaChannelBits)
					continue;
	...
	public void BuildDepthStencilFormatList(DeviceCombo deviceCombo)
	{
		...
		{
			if (GetDepthBits(depthStencilFmt) < AppMinDepthBits)
				continue;
			if (GetStencilBits(depthStencilFmt) < AppMinStencilBits)
				continue;
